...
public class CartController {
	...
	public String thanhToan(HttpServletRequest request, Model model) {
		...
		try {
			...
			for (int i = 0; i < tensp.length; i++) {
				for (int j = 0; j < lokk.size(); j++) {
					// Kiểm tra xem có phần tử nào trong mảng ko
					if (lokk.get(j).getSanphamchitiet().getMact().getName().equals(tensp[i])) {
						lokk.get(j).getSanphamchitiet().setAmount(ints[i]);
					}
				}
	...
	public ModelAndView thanhToanTcGh(HttpServletRequest request, Model model) {
		...
		KhachHang khachhang = new KhachHang(fname, usname, pass, diachi, call, email, ngay, gioitinh, 1);
	...
	@PostMapping(value = "/tang-soluong")
	public String thayDoiSoLuong(@RequestParam(value = "quantity") int soluong) {
